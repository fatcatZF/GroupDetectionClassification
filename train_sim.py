...
parser.add_argument("--save-folder", type=str, default="logs/sim",
...
encoder = EncoderSym(args.dims, args.edge_dim, args.encoder_hidden, 1,
...
def train(epoch, best_F1):
    ...
    for idx in training_indices:
        ...
        #shape: [n_nodes, n_timesteps, n_in]
        ...
        output = torch.tanh(logits.flatten())
        #shape: [n_edges]
        target = 2*(label.flatten()-0.5)
        ...
        sims_label_G = nx.from_numpy_array(sims_label)
        clusters_label = list(connected_components(sims_label_G))
        clusters_label = [list(c) for c in clusters_label]

        sims_predict = label2mat(output, num_atoms)
        clusters_predict, _ = greedy_approximate_best_clustering(sims_predict)
